...
public final class MainDisplayable extends PaintableObject implements Runnable, CommandListener, ItemStateListener
{
	...
	public final void load() throws IOException
	{
		...
		if(is != null)
		{
			...
			while(true)
			{
				...
				{
					tilesTypes[Integer.parseInt(line[1])] = (byte)Integer.parseInt(line[2]);
				}
			}
	...
	public final void loadMainGameResources() throws IOException
	{
		...
		if(mode != 1)
		{
			...
			Renderer.loadResources();
			sprUnitIcons = new Sprite[5][12];

			byte fraction;
			byte unit;
			SpriteFrame[] var5;

			for(fraction = 0; fraction < 5; ++fraction)
			{
				...
				for(unit = 0; unit < 12; ++unit)
				{
					...
					else
					{
						...
						for(int var6 = 0; var6 < iconCount; ++var6)
						{
							var5[var6] = unitIcons.frames[var6 * 12 + unit];
						}
...
//		}
	...
	public final Sprite createSimpleSparkSprite(Sprite var1, int var2, int var3, int var4, int var5, int var6, int var7)
	{
		Sprite var8;
		(var8 = Sprite.createSimpleSparkSprite(var1, var4, var5, 0, var6, var7, (byte)0)).setPosition(var2, var3);
								...
								else if(seltext != null && action == 0)
								{
									...
									else if(!seltext.equals(MENU_ITEMS_TEXT[1]) && auxdisp != auxSelectLevelList)
									{
										...
										else if(!seltext.equals(PaintableObject.getLocaleString(6)))
										{
											...
											{
												setMapElement((byte)tilesCount, currentSelectedUnit.currentMapPosX, currentSelectedUnit.currentMapPosY);
		...
		try
		{
			...
			for(mx = 0; mx < mapWidth; ++mx)
			{
				for(short my = 0; my < mapHeight; ++my)
				{
					...
					if(mapData[mx][my] >= tilesCount || mapData[mx][my] == 27)
					{
	...
	public final void updateIntroTransition()
	{
		...
		{
			sfmGameLogo = new SpriteFrame("logo");
			introTransMode = 3;
			combatDrapValue = 40;
		}
		...
		switch(introTransMode)
		{
			...
			case 3:
				if((combatDrapValue += 1) > 40)
				{
					try
					{
						sfmSplash = new SpriteFrame("splash");
					...
					else
					{
						...
						if(gameState == 8)
						{
							...
							else
							{
								...
								{
									if((var20 = (Unit)units.elementAt(var1)).unitState != 3 && currentTurningPlayer == var20.fractionPosInTurnQueue && (getTileType(var20.currentMapPosX, var20.currentMapPosY) == 7 || sub_e68(var20.currentMapPosX, var20.currentMapPosY, playerTeams[var20.fractionPosInTurnQueue])) && var20.health < 100)
									{
										int var25;
										if((var25 = 100 - var20.health) > 20)
										{
											var25 = 20;
										}
										...
										var20.health += var25;
										Sprite var10;
										(var10 = Sprite.createGraphicTextSprite("+" + var25, 0, -4, (byte)1)).setPosition(var20.currentX + var20.width / 2, var20.currentY + var20.height);
	...
	public final void setIntroMode(int var1, int var2, int var3)
	{
		...
		try
		{
			sprIntro = new SpriteFrame("intro" + var1);
		}
		catch(Exception var4)
		{
			;
	...
	public final void paint(Graphics g)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					Unit var4 = (Unit)units.elementAt(var2);

					if(var4.unitState == 3)
					{
						sprTombStone.paint(g, mapX + var4.currentX, mapY + var4.currentY);
					}
					else if(var4 != currentSelectedUnit)
					{
						var4.paint(g, mapX, mapY);
					}
				}
	...
	{
		if(mapData[var1][var2] >= tilesCount)
		{
			setMapElement((byte)(tilesCount + var3 * 2 + (mapData[var1][var2] - tilesCount) % 2), var1, var2);
		}
	}
	...
	{
		return var3[var1][var2] >= tilesCount ? (var3[var1][var2] - tilesCount) / 2 : -1;
	}
	...
	{
		int var4;
		return (var4 = getFractionPosInTurnQueue(getBuildingFraction(var1, var2))) > -1 ? var3 == playerTeams[var4] : false;
	}
	...
	private final boolean evaluateScriptTest(String test, int a, int b)
	{
		if(test.equals("="))
		{
			return a == b;
