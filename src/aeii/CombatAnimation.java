...
public final class CombatAnimation
{
	...
	public CombatAnimation(MainDisplayable var1, Unit var2, CombatAnimation var3)
	{
		currentMainDisplayable = var1;
		currentUnit = var2;
		unitType = var2.type;
		currentAttackerSideCombatAnimation = var3;

		initialUnitHealth = (byte)var2.health;
		currentUnitHealth = initialUnitHealth;

		initialUnitCharCount = (byte)var2.getCharacterCount();
		...
		{
			var4 = var1.halfWidth_;
			mirror = 1;
			combatState = 0;
		}

		tileType = var1.getTileType(var2.currentMapPosX, var2.currentMapPosY);

		byte var5 = BACKGROUNDS[tileType];
		byte var6 = FOREGROUNDS[tileType];

		if(var3 != null && var6 == FOREGROUNDS[var3.tileType])
		{
			sfmTerrainForeground = new SpriteFrame[var3.sfmTerrainForeground.length];
			System.arraycopy(var3.sfmTerrainForeground, 0, sfmTerrainForeground, 0, sfmTerrainForeground.length);
		}
		else
		{
			sfmTerrainForeground = (new Sprite(TERRAIN_NAMES[var6])).frames;
		}
		...
		try
		{
			if(var3 != null && var5 == BACKGROUNDS[var3.tileType])
			{
				sfmTerrainBackground = var3.sfmTerrainBackground;
			}
			else
			{
				sfmTerrainBackground = new SpriteFrame(TERRAIN_NAMES[var5] + "_bg");
			}
		...
		tilesCountX = var1.halfWidth_ / sfmTerrainForeground[0].imgwidth;

		if(var1.halfWidth_ % sfmTerrainForeground[0].imgwidth != 0)
		{
			++tilesCountX;
		}

		tilesCountY = (var1.var_4138 - backgroundHeight) / sfmTerrainForeground[0].imgheight;

		if((var1.var_4138 - backgroundHeight) % sfmTerrainForeground[0].imgheight != 0)
		{
			++tilesCountY;
		}
		...
		sprUnit = new Sprite(UNITS_NAMES[unitType], var1.fractionsTurnQueue[var2.fractionPosInTurnQueue]);

		if(var3 != null && var3.unitType == unitType)
		{
			if(var3.sprKingSlash != null)
			{
				sprKingSlash = new Sprite(var3.sprKingSlash);
			}

			if(var3.sprAttackSideEffect != null)
			{
				sprAttackSideEffect = new Sprite(var3.sprAttackSideEffect);
			}

			if(var3.sprVictimSideEffect != null)
			{
				sprVictimSideEffect = new Sprite(var3.sprVictimSideEffect);
			}

			if(var3.sprKingHeads != null)
			{
				sprKingHeads = new Sprite(var3.sprKingHeads);
				sprKingHeadsBack = new Sprite(var3.sprKingHeadsBack);
			}
		}
		...
		{
			sprKingHeads.setFrameSequence(var2.kingVariant, shiftFrameSequence);
			sprKingHeads.setCurrentFrame(var2.kingVariant);
			sprKingHeadsBack.setFrameSequence(0, shiftFrameSequence);
			sprKingHeadsBack.setCurrentFrame(var2.kingVariant);
		}

		characterPositions = new int[var2.characterData.length][2];

		for(var7 = 0; var7 < characterPositions.length; ++var7)
		{
			characterPositions[var7][0] = var2.characterData[var7][0] * var1.width / 128;

			if(mirror == 1)
			{
				characterPositions[var7][0] = var1.halfWidth_ - characterPositions[var7][0] - sprUnit.width + var4;
			}

			characterPositions[var7][1] = var2.characterData[var7][1] * var1.var_4138 / 128 - sprUnit.height;
		}
		...
		for(var7 = 0; var7 < initialUnitCharCount; ++var7)
		{
			...
			var1.addCombatAnimationSprite(sprUnitCharacters[var7]);
	...
	public final void paintTerrain(Graphics g, int tx, int ty)
	{
		...
		if(sfmTerrainBackground != null)
		{
			...
			for(var8 = currentMainDisplayable.halfWidth_ / var6; var7 < var8; ++var7)
			{
