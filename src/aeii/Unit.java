...
public final class Unit extends Sprite
{
			...
			for(int var12 = var8; var12 <= var10; ++var12)
			{
				...
				if((var13 = Math.abs(var11 - var1) + Math.abs(var12 - var2)) >= minRange && var13 <= maxRange)
				{
					...
					if(var5 == 0)
					{
						...
						else if(type == 7 && currentMainDisplayable.getTileType(var11, var12) == 8 && currentMainDisplayable.mapData[var11][var12] >= currentMainDisplayable.tilesCount && !currentMainDisplayable.sub_e68(var11, var12, currentMainDisplayable.playerTeams[fractionPosInTurnQueue]))
						{
							Unit var15;
	...
	public final void paintEx(Graphics g, int x, int y, boolean var4)
	{
		if(unitState != 4)
		{
			int var6;
			...
			else if(!var4 && unitState != 2)
			{
				super.paint(g, x, y);
			}
			else
			{
				currentMainDisplayable.sprUnitIcons[0][type].paint(g, super.currentX + x, super.currentY + y);
			}
